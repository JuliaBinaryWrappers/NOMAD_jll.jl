# Autogenerated wrapper script for NOMAD_jll for x86_64-apple-darwin14-cxx03
export libsgtelib, libnomadAlgos, libnomadInterface, libnomadUtils, nomad, libnomadEval
...
const libsgtelib_splitpath = ["lib", "libsgtelib.dylib"]
...
const libsgtelib = "@rpath/libsgtelib.dylib"


# Relative path to `libnomadAlgos`
const libnomadAlgos_splitpath = ["lib", "libnomadAlgos.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadAlgos = "@rpath/libnomadAlgos.dylib"


# Relative path to `libnomadInterface`
const libnomadInterface_splitpath = ["lib", "libnomadInterface.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadInterface = "@rpath/libnomadInterface.dylib"


# Relative path to `libnomadUtils`
const libnomadUtils_splitpath = ["lib", "libnomadUtils.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadUtils = "@rpath/libnomadUtils.dylib"
...
const libnomadEval_splitpath = ["lib", "libnomadEval.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadEval = "@rpath/libnomadEval.dylib"
