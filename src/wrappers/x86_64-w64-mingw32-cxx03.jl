...
const libnomadAlgos_splitpath = ["bin", "libnomadAlgos.dll"]
...
const libnomadAlgos = "libnomadAlgos.dll"


# Relative path to `libnomadEval`
const libnomadEval_splitpath = ["bin", "libnomadEval.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadEval = "libnomadEval.dll"


# Relative path to `libnomadInterface`
const libnomadInterface_splitpath = ["bin", "libnomadInterface.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadInterface = "libnomadInterface.dll"


# Relative path to `libnomadUtils`
const libnomadUtils_splitpath = ["bin", "libnomadUtils.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnomadUtils = "libnomadUtils.dll"


# Relative path to `libsgtelib`
const libsgtelib_splitpath = ["bin", "libsgtelib.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libsgtelib = "libsgtelib.dll"


# Relative path to `nomad`
const nomad_splitpath = ["bin", "nomad.exe"]
